...
class MyPageControllerTest {
	...
	// テスト対象へMockオブジェクトを注入
	@InjectMocks
	private MyPageController myPageCntroller;
	...
	@Test
	@DisplayName("ユーザ情報を変更する際確認画面に遷移できるか確認(リンクから飛んできてFormの値がnullの場合)")
...
//				.param("telephone", null)
	...
	@Test
	@DisplayName("ユーザ情報を変更する際確認画面に遷移できるか確認(バリデーションから飛んできてFormの値がなにかしら入っている場合)")
...
//	@Test
//	@DisplayName("ユーザ情報を更新した際、次の画面に遷移できるか確認・sessionに値が入っているのか確認")
//	void test5() throws Exception {
//		User updateUser = new User();
//		updateUser.setId(null);
//		when(userService.update(updateUser)).thenReturn();
//	}

	// ↓エラー出ないけどうまくいってないよ～～～～
	@Test
	@DisplayName("マイクーポンを表示する(1枚以上所持している場合)")
	...
	void test6() throws Exception {
		...
		coupon.setUserId(1);
		usersCouponList.add(coupon);
		// doメソッド系
		// when(モックオブジェクト).メソッド(任意の引数)
		// doReturn(返り値)： スタブメソッドの返り値を指定する
		// doメソッド系ではwhenメソッドより先に実行結果を定義するメソッドが来る
		// モック化されたcouponServiceのfindAllUsersCouponメソッドに「ユーザーid
		// 6」を渡したときの返り値は「userCouponList」
		...
		// mvcResultにテストの結果を返す
		MvcResult mvcResult = mockMvc.perform(get("/myPage/myCoupon").session(mockSession)).andExpect(status().isOk())
				.andExpect(view().name("myCoupon")).andReturn();
	...
	@Test
	@DisplayName("マイクーポンを表示する（所持していない場合）")
	...
	void test7() throws Exception {
		...
		doReturn(usersCouponList).when(couponService).findAllUsersCoupon(1);
		...
		// String message = (String) mav.getModel().get("nonCoupon");
		// 正しく格納されているか確認。
	...
	@Test
	@DisplayName("お届け先情報追加画面(0~2件登録してある時)")
	void test8() throws Exception {
		List<Addressee> addresseeList = new ArrayList<>();
		doReturn(addresseeList).when(addresseeService).findAddresseeByUserId(6);
		...
		MvcResult result = mockMvc.perform(get("/myPage/registerShow").session(mockSession)).andExpect(status().isOk())
				.andExpect(view().name("addressee_register")).andReturn();

		HttpSession session = result.getRequest().getSession();
		assertEquals(addresseeList.getClass(), session.getAttribute("addresseeList").getClass());
